...
def whatwillyoudo():
    ...
    while (castleHealth > 0 and men > 0):
        decisionInput = input(
            "Type 1 to shoot arrows, Type 2 to send a certain amount of men to make a rush towards the castle, and Type 3 to fire the cannon")

        if decisionInput == "1":
            menkilled = random.randint(0, wave * 4)
            men = men - menkilled
            arrowdamagedone = 1 / 2 * men * 1
        ...
        if decisionInput == "3":
            count = count + 1
            if count >= 2:
                print("You have already used the cannon once!")

            elif count == 1:
                cannonChance = random.randint(0, 2)
                menkilled = random.randint(0, wave * 3)
                men = men - menkilled
                if cannonChance == 2:
                    print("The cannon missed! The castle now has " + str(castleHealth) + " health.")
                    print("You have lost " + str(menkilled) + " men from return fire. You now have " + str(men) + ".")
                else:
                    castleHealth = castleHealth - cannonDamage
                    count = count + 1
                    print("The cannon struck! The castle now has " + str(castleHealth) + " health")
                    print("You have lost " + str(menkilled) + " men from return fire. You now have " + str(men) + ".")
    if castleHealth <= 0:
        print("Castle " + str(wave) + " sieged!")
        print("Starting next wave. . .")
        startGame(wave + 1, castleHealth, men, arrowDamage, cannonDamage)  # problem with next wave
    if men <= 0:
        print("All of your men died! GAME OVER!")
        exit


def startGame(wave, castleHealth, men, arrowDamage, cannonDamage):
    castleHealth = 100 * wave
    men = 25 * wave
    print("WAVE " + str(wave))
    print("Men = " + str(men))
...
userInput = input('Welcome to S.I.E.G.E alpha v1.3. Press 1 to continue')
if userInput == "1":
    ...
    if userInput == "1":
        input('You are raiding the castles of a neighboring kingdom...' "\n"
              ...
              'Or Press 3 to fire the cannon (hint: the cannon has 1 use per round and high a 1/3 chance of causing high damage).' "\n"
        ...
        if userInput == "1":
            print('Starting. . .')
            startGame(1, castleHealth * wave, men, arrowDamage, cannonDamage)
        else:
            print('Invalid Input.')
    elif userInput == "2":
        print('Starting. . .')
